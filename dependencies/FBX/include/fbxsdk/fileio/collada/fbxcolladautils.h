...
#include "fbxcolladatokens.h"
#include "fbxcolladaiostream.h"
...
const FbxString COLLADA_ID_PROPERTY_NAME = "COLLADA_ID";
...
typedef FbxScopedPtr<xmlNode, XmlNodeDeletionPolicy> XmlNodePtr;
                                         ...
                                         FbxString & pData)
{
    ...
    xmlChar * lPropertyValue = xmlGetProp(pElement,
        (const xmlChar *)pAttributeName);
    if (lPropertyValue)
    {
...
xmlNode * DAE_GetSourceWithSemantic(xmlNode * pConsumerElement, const char * pSemantic,
                                    const std::map< FbxString, xmlNode* > & pSourceElements);
