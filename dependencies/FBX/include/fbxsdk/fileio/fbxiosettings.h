...
public:
	...
	//! If pTimeMode is set to FbxTime::eDefaultMode, pCustomFrameRate is used instead.
    void SetTimeMode(FbxTime::EMode pTimeMode, double pCustomFrameRate = 0.0);

	//! A time mode set to FbxTime::eDefaultMode means a custom frame rate is set.
    FbxTime::EMode GetTimeMode(){ return mTimeMode; }

	//! Get frame period associated with time mode or a custom frame period if time mode is set to FbxTime::eDefaultMode.
...
// this class is located in the UserDataPtr of each FbxIOSettings properties
class PropInfo
{
public:
	PropInfo();
...
#ifndef DOXYGEN_SHOULD_SKIP_THIS
public:
	PropInfo* GetPropInfo(FbxProperty &pProp);

	EFbxUILanguage UILanguage;
	FbxString GetLanguageLabel(FbxProperty& pProp);
	void SetLanguageLabel(FbxProperty& pProp, FbxString& pLabel);
	EFbxUILanguage Get_Max_Runtime_Language(FbxString pRegLocation);
...
protected:
	FbxIOSettings(FbxManager& pManager,const char* pName);
