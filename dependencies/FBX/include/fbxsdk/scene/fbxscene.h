...
public:
    ...
    */
    ...
    /** Get number of textures (type FbxTexture).
	  ...
	  * \code
	  * int lNbFileTextures = lScene->GetSrcObjectCount(FBX_TYPE(FbxFileTexture));
	  * int lNbLayeredTextures = lScene->GetSrcObjectCount(FBX_TYPE(FbxLayeredTexture));
	  * int lNbProceduralTextures = lScene->GetSrcObjectCount(FBX_TYPE(FbxProceduralTexture));
	  * \endcode
	  */
    ...
    /** Get the texture at the given index. pIndex must be between 0 and GetTextureCount().
	  ...
	  * \code
	  * FbxFileTexture* lFileTexture = lScene->GetSrcObject(FBX_TYPE(FbxFileTexture), i);
	  * FbxLayeredTexture* lLayeredTexture = lScene->GetSrcObject(FBX_TYPE(FbxLayeredTexture), i);
	  * FbxProceduralTexture* lProceduralTexture = lScene->GetSrcObject(FBX_TYPE(FbxProceduralTexture), i);
	  * \endcode
      */
...
private:
    FbxScene(FbxManager& pManager, char const* pName);
...
public:
    ...
    /**
     * Recursively convert transform inherit type from eInheritRrs to eInheritRSrs by compensating object scale.
     ...
     * The relevant data that is contained in these objects have been moved into the FbxGlobalSettings(). 
