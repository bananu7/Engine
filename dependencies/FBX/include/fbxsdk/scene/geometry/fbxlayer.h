...
/** Layer element for mapping materials (FbxSurfaceMaterial) to a geometry.
  ...
  * or the more generic calls to GetSrcObjectCount(FBX_TYPE(FbxSurfaceMaterial)) and 
  * GetSrcObject(FBX_TYPE(FbxSurfaceMaterial), index)
  *
  * For example:
  *
  * \code
		...
		for( i = 0; i < mDataTypes.GetCount(); ++i )
		{
			...
			if (dst)
			{
				switch( mDataTypes[i].GetType() )
				{
					...
					//case eFbxDouble3:	mDirectArray[i] = new FbxArray< FbxDouble3 >();	break;  
					//case eFbxDouble4:	mDirectArray[i] = new FbxArray< FbxDouble4 >();	break;
					//case eFbxDouble4x4:	mDirectArray[i] = new FbxArray< FbxDouble4x4 >();	break;  
					default:
