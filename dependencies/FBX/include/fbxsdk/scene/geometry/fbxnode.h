...
public:
	...
	//@{
		/** Set the node Visibility value from the boolean parameter. 
		  ...
		  * {
		  *     FbxDouble val(pIsVisible);
		  *     FbxPropertySet <FbxDouble> (Visibility, val);
		  * }
		  * \endcode
		  *
	...
	/**
	  ...
	  *        FbxAMatrix lSourceR;
	  *        FbxAMatrix lR(lRV.mData[0], lRV.mData[1], lRV.mData[2]);
	  *        FbxVector4 res = prV;
	  *
	...
	/** Find out start and end time of the animation curves for this node (and its children).
	  ...
	  *               to initialize pSpan with FbxTimeSpan(FBXSDK_TIME_INFINITE, FBXSDK_TIME_MINUS_INFINITE).
...
protected:
	FbxNode(FbxManager& pManager, char const* pName);
...
private:
	...
	void				ConvertPivotAnimationRecursiveWrapper(const char * pAnimStackName, EPivotSet pConversionTarget, double pFrameRate, bool pKeyReduce);
	FbxNode&			DeepCopy(const FbxNode& pNode, bool pCopyNodeAttribute);

	FbxTransform		mTransform;
	FbxNode_internal*	mPH;
	...
	friend class FbxNode_internal;
	friend class FbxNodeEvalState;
