...
class   FbxPropertyInfo;
class   FbxPropertyValue;
class   FbxPropertyPage;
class   FbxPropertyHandle;

typedef FbxPropertyPage    FbxTypeInfo;
typedef FbxPropertyHandle  FbxTypeInfoHandle;
...
public:
    ...
    inline FbxPropertyFlagsExt& operator=( const FbxPropertyFlagsExt& pOther ) { mFlagData = pOther.mFlagData; return *this; }
    ...
    inline FbxPropertyAttr::EFlags GetMask() const { return (FbxPropertyAttr::EFlags) (mFlagData >> 16); }

    inline FbxPropertyAttr::EFlags GetFlags() const { return (FbxPropertyAttr::EFlags) (mFlagData & 0x0000FFFF); }
...
public:
    FBXSDK_FRIEND_NEW();
    static FbxPropertyInfo* Create(const char* pName, FbxTypeInfo* pTypeInfo)         { return FbxNew< FbxPropertyInfo >(pName,pTypeInfo); }
    ...
    FbxTypeInfo*       GetTypeInfo() const             { return mTypeInfo; }
...
private:
	 FbxPropertyInfo(const char* pName, FbxTypeInfo* pTypeInfo)
        : mRef(0)
        , mName(pName)
        , mType(eFbxUndefined)
        , mTypeInfo(pTypeInfo)
    ...
    FbxPropertyInfo(FbxStringSymbol pName,FbxTypeInfo *pTypeInfo)
        : mRef(0)
        , mName(pName)
        , mType(eFbxUndefined)
        , mTypeInfo(pTypeInfo)
    ...
    EFbxType                    mType;
    FbxTypeInfo*               mTypeInfo;      // Sub property
    int                         mUserTag;
    void*                       mMinMaxValue[eValueCount];  
    void*                       mUserData;
    FbxConnectionPointFilter*  mFilter;
    FbxDefaultScopedPtr<FbxStringList>     mEnumList;
};
...
public:
    ...
    static FbxTypeInfo*     Create (const char* pName, FbxTypeInfo* pTypeInfo)    { return FbxNew< FbxPropertyPage >(pName,pTypeInfo); }
    static FbxTypeInfo*     Create (const char* pName, EFbxType    pType=eFbxUndefined)     { return FbxNew< FbxPropertyPage >(pName,pType);     }
    void                     Destroy() { FbxDelete(this); }
    ...
    FbxTypeInfo* GetTypeInfo(FbxInt pId=FBXSDK_PROPERTY_ID_ROOT)
    {
		FbxPropertyInfo* lPropertyInfo = GetPropertyItem( FBX_TYPE(FbxPropertyInfo),pId );
    ...
    FbxInt Add(FbxInt pParentId, const char* pName, FbxTypeInfo* pTypeInfo)
    {
    ...
    */
    ...
    FbxInt FastFind (FbxInt pId, const char* pName, FbxTypeInfo* pTypeInfo, bool pCaseSensitive)
    {
        ...
        if( mNameMap.mSecond.GetSize() > 0 )
        {
            ...
            KNameMap::RecordType* lIterator = mNameMap.mSecond.Find( FbxNameMapKey( pId, pName ) );
            if( !lIterator )
            {
            ...
            else
            {
                ...
                if (lId != FBXSDK_PROPERTY_ID_NULL && pTypeInfo)
                {
                    ...
                    if (lPropertyInfo)
                    {
                        FbxTypeInfo* lTypeInfo2 = lPropertyInfo->GetTypeInfo();
    ...
    FbxInt Find (FbxInt pId, const char* pName, FbxTypeInfo* pTypeInfo, bool pCaseSensitive, const char* pChildrenSeparators )
    {
...
protected:
    ...
    FbxPropertyPage(const char* pName, FbxTypeInfo* pTypeInfo)
        : mInstanceOf(0)
        , mDataPtr(0)
        , mPropNextId(0)
    {
    ...
    ~FbxPropertyPage()
    {
        ...
        KEntryMap::Iterator lElement = mEntryMap.Begin();
...
private:
    ...
    */
    FbxInt GetMinimumPropertyId(FbxInt pId, bool pIncrementIfNone = true) const
    {
        ...
        const KEntryMap::RecordType* lElement = mEntryMap.UpperBound(pId);
        if (NULL != lElement)
        {
    ...
    */
    ...
    {
        int lCount = 0;
        const KEntryMap::RecordType* lElement = mEntryMap.Maximum();
    ...
    {
        const KEntryMap::RecordType* lElement = mEntryMap.Find(pIndex);
        if (NULL != lElement)
        {
            if( pFoundIn )
            {
    ...
    FbxInt Add(FbxInt pParentId,FbxPropertyInfo* pInfo,FbxPropertyValue* pValue,FbxPropertyConnect* pConnect,bool pRecursive=true)
    {
        ...
        if (pRecursive) {
            FbxTypeInfo* lTypeInfo = pInfo->GetTypeInfo();
            if (lTypeInfo) {
				FbxInt lChildId;
    ...
    // Property management
    typedef FbxMap<FbxInt, FbxPropertyEntry*, FbxLessCompare<FbxInt>, FbxHungryAllocator> KEntryMap;
    KEntryMap mEntryMap;
    ...
    // speed up structure
    typedef FbxMap< FbxNameMapKey, FbxInt, FbxNameMapCompare > KNameMap;
    typedef FbxPair< unsigned int, KNameMap > KNameLookup;
    KNameLookup     mNameMap;
