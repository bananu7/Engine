...
class FbxQueryOperator;
class FbxQueryOperatorUnary;
class FbxQueryClassId;
class FbxQueryIsA;
class FbxQueryIsProperty;
...
public:
    ...
    /**This compares whether two FbxQuery are the same, NOT whether the query
...
/** Defines a filtering criteria for a query of objects, connections
  ...
  * \see FbxCollection::GetMemberCount(FbxCriteria) const
  * \see FbxLibrary::ImportAssets(FbxLibrary*, const FbxCriteria&)
  * \nosubgrouping
  */
...
private:
    ...
    public:
        ...
        FbxCriteria* GetObjectType(FbxClassId pClassId);
...
public:
    ...
    static FbxQueryIsA* Create(FbxClassId pClassId);       
    virtual FbxInt GetUniqueId() const{ return FBXSDK_QUERY_UNIQUE_ID+4; }
    virtual bool IsValid(const FbxProperty& pProperty) const;
    virtual bool IsEqual(FbxQuery* pOtherQuery) const;

protected:
    FbxQueryIsA(FbxClassId pClassId);
