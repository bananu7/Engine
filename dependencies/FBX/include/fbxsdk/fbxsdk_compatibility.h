...
//	KFbxConnectSrc						->	FbxConnectSrc
//	KFbxConnectDst						->	FbxConnectDst
...
FBX_DEPRECATED inline void KFbxUTF8ToWidechar(const char* pInUTF8, wchar_t*& pOutWideChar) { FbxUTF8ToWC(pInUTF8, pOutWideChar); }
FBX_DEPRECATED inline void KFbxWideCharToUTF8(const wchar_t* pInWideChar, char*& pOutUTF8) { FbxWCToUTF8(pInWideChar, pOutUTF8); }
FBX_DEPRECATED inline void RemoveChar(FbxString& pString, char pToRemove){ FbxRemoveChar(pString, pToRemove); }
#if defined(FBXSDK_ENV_WIN)
	FBX_DEPRECATED inline void KFbxWideCharToAnsi(const wchar_t* pInWideChar, char*& pOutANSI){ FbxWCToAnsi(pInWideChar, pOutANSI); }
	FBX_DEPRECATED inline void KFbxAnsiToWideChar(const char* pInANSI, wchar_t*& pOutWideChar){ FbxAnsiToWC(pInANSI, pOutWideChar); }
	FBX_DEPRECATED inline void KFbxAnsiToUTF8(const char* pInANSI, char*& pOutUTF8){ FbxAnsiToUTF8(pInANSI, pOutUTF8); }
	FBX_DEPRECATED inline void KFbxUTF8ToAnsi(const char* pInUTF8, char*& pOutANSI){ FbxUTF8ToAnsi(pInUTF8, pOutANSI); }
#endif

FBX_DEPRECATED inline void KRegisterAssertStubFct(FbxAssertProc pAssertProc){ FbxAssertSetProc(pAssertProc); }
FBX_DEPRECATED inline KString kGetTempPath(){ return FbxGetSystemTempPath(); }
FBX_DEPRECATED inline KString KFbxExtractFileName(const char* pFileName, bool pWithExtension){ return FbxPathUtils::GetFileName(pFileName, pWithExtension); }
FBX_DEPRECATED inline KString KFbxCleanPath(const char* pFilePath){ return FbxPathUtils::Clean(pFilePath); }
FBX_DEPRECATED inline KString KFbxExtractFileExtension(const char* pFilePath){ return FbxPathUtils::GetExtensionName(pFilePath); }
FBX_DEPRECATED inline KString KFbxExtractDirectory(const char* pFilePath){ return FbxPathUtils::GetFolderName(pFilePath); }
FBX_DEPRECATED inline void KTime_SetGlobalTimeMode(KTime::EMode pTimeMode, double pFrameRate=0.0){ FbxSetGlobalTimeMode(pTimeMode, pFrameRate); }
FBX_DEPRECATED inline double KTime_GetFrameRate(KTime::EMode pTimeMode){ return FbxGetFrameRate(pTimeMode); }
FBX_DEPRECATED inline kLibHandle KLibraryGetProc(kLibHandle pLibHandle, const char* pProcName){ return FbxModuleGetProc(pLibHandle, pProcName); }
